// src/vendor/hanzi-writer.esm.js
import { state } from '../state.js';

  export var _globalObj$requestAni;
  export var _globalObj$cancelAnim;
  export var _globalObj$navigator;
  export var globalObj = typeof window === "undefined" ? global : window;
  export var performanceNow = globalObj.performance && (() => globalObj.performance.now()) || (() => Date.now());
  export var requestAnimationFrame = ((_globalObj$requestAni = globalObj.requestAnimationFrame) === null || _globalObj$requestAni === void 0 ? void 0 : _globalObj$requestAni.bind(globalObj)) || ((callback) => setTimeout(() => callback(performanceNow()), 1e3 / 60));
  export var cancelAnimationFrame = ((_globalObj$cancelAnim = globalObj.cancelAnimationFrame) === null || _globalObj$cancelAnim === void 0 ? void 0 : _globalObj$cancelAnim.bind(globalObj)) || clearTimeout;
  export function arrLast(arr) {
    return arr[arr.length - 1];
  }
  export var fixIndex = (index, length2) => {
    if (index < 0) {
      return length2 + index;
    }
    return index;
  };
  export var selectIndex = (arr, index) => {
  ...
  export var trim = (string) => string.replace(/^\s+/, "").replace(/\s+$/, "");
  ...
  export var ua = ((_globalObj$navigator = globalObj.navigator) === null || _globalObj$navigator === void 0 ? void 0 : _globalObj$navigator.userAgent) || "";
  export var isMsBrowser = ua.indexOf("MSIE ") > 0 || ua.indexOf("Trident/") > 0 || ua.indexOf("Edge/") > 0;
  export var noop = () => {
  };
  export var RenderState = class {
  ...
  export var subtract = (p1, p2) => ({
    x: p1.x - p2.x,
    y: p1.y - p2.y
  });
  export var magnitude = (point) => Math.sqrt(Math.pow(point.x, 2) + Math.pow(point.y, 2));
  export var distance = (point1, point2) => magnitude(subtract(point1, point2));
  export var equals = (point1, point2) => point1.x === point2.x && point1.y === point2.y;
  export var round = (point, precision = 1) => {
  ...
  export var cosineSimilarity = (point1, point2) => {
    const rawDotProduct = point1.x * point2.x + point1.y * point2.y;
    return rawDotProduct / magnitude(point1) / magnitude(point2);
  };
  export var _extendPointOnLine = (p1, p2, dist) => {
    const vect = subtract(p2, p1);
    const norm2 = dist / magnitude(vect);
    return {
      x: p2.x + norm2 * vect.x,
      y: p2.y + norm2 * vect.y
    };
  };
  export var frechetDist = (curve1, curve2) => {
  ...
  export var subdivideCurve = (curve, maxLen = 0.05) => {
  ...
  export var outlineCurve = (curve, numPoints = 30) => {
  ...
  export var normalizeCurve = (curve) => {
  ...
  export var extendStart = (points, dist) => {
  ...
  export var preScaledWidth = to.x - from.x;
  export var preScaledHeight = to.y - from.y;
  export var Positioner = class {
    constructor(options) {
      const {
        padding,
        width,
  ...
  export var COSINE_SIMILARITY_THRESHOLD = 0;
  export var START_AND_END_DIST_THRESHOLD = 250;
  export var FRECHET_THRESHOLD = 0.4;
  export var MIN_LEN_THRESHOLD = 0.35;
  export function strokeMatches(userStroke, character, strokeNum, options = {}) {
  ...
  export var startAndEndMatches = (points, closestStroke, leniency) => {
  ...
  export var getEdgeVectors = (points) => {
    const vectors = [];
    let lastPoint = points[0];
  ...
  export var directionMatches = (points, stroke) => {
  ...
  export var lengthMatches = (points, stroke, leniency) => {
    return leniency * (length(points) + 25) / (stroke.getLength() + 25) >= MIN_LEN_THRESHOLD;
  };
  export var stripDuplicates = (points) => {
  ...
  export var SHAPE_FIT_ROTATIONS = [Math.PI / 16, Math.PI / 32, 0, -1 * Math.PI / 32, -1 * Math.PI / 16];
  export var shapeFit = (curve1, curve2, leniency) => {
  ...
  export var getMatchData = (points, stroke, options) => {
    const {
      leniency = 1,
  ...
  export function getPartialValues(startValues, endValues, progress) {
    const target = {};
  ...
  export function isAlreadyAtEnd(startValues, endValues) {
    for (const key in endValues) {
      const endValue = endValues[key];
      const startValue = startValues === null || startValues === void 0 ? void 0 : startValues[key];
      if (endValue >= 0) {
  ...
  export var ease = (x) => -Math.cos(x * Math.PI) / 2 + 0.5;
  export var showStrokes = (charName, character, duration) => {
  ...
  export var showCharacter = (charName, character, duration) => {
  ...
  export var hideCharacter = (charName, character, duration) => {
    return [new Mutation(`character.${charName}.opacity`, 0, {
      duration,
      force: true
    }), ...showStrokes(charName, character, 0)];
  };
  export var updateColor = (colorName, colorVal, duration) => {
    return [new Mutation(`options.${colorName}`, colorVal, {
      duration
    })];
  };
  export var highlightStroke = (stroke, color, speed) => {
  ...
  export var animateStroke = (charName, stroke, speed) => {
    const strokeNum = stroke.strokeNum;
    const duration = (stroke.getLength() + 600) / (3 * speed);
    return [new Mutation(`character.${charName}`, {
      opacity: 1,
  ...
  export var animateSingleStroke = (charName, character, strokeNum, speed) => {
    const mutationStateFunc = (state2) => {
      const curCharState = state2.character[charName];
      const mutationState = {
        opacity: 1,
        strokes: {}
      };
  ...
  export var showStroke = (charName, strokeNum, duration) => {
  ...
  export var animateCharacter = (charName, character, fadeDuration, speed, delayBetweenStrokes) => {
  ...
  export var animateCharacterLoop = (charName, character, fadeDuration, speed, delayBetweenStrokes, delayBetweenLoops) => {
  ...
  export var startQuiz = (character, fadeDuration, startStrokeNum) => {
  ...
  export var startUserStroke = (id, point) => {
  ...
  export var updateUserStroke = (userStrokeId, points) => {
    return [new Mutation(`userStrokes.${userStrokeId}.points`, points, {
      force: true
    })];
  };
  export var hideUserStroke = (userStrokeId, duration) => {
    return [new Mutation(`userStrokes.${userStrokeId}.opacity`, 0, {
      duration
    })];
  };
  export var removeAllUserStrokes = (userStrokeIds) => {
  ...
  export var highlightCompleteChar = (character, color, duration) => {
  ...
  export var StrokeRendererBase = class _StrokeRendererBase {
    constructor(stroke) {
      this.stroke = stroke;
  ...
  export var STROKE_WIDTH = 200;
  export var StrokeRenderer = class extends StrokeRendererBase {
    constructor(stroke) {
      super(stroke);
  ...
  export var CharacterRenderer = class {
  ...
  export var UserStrokeRenderer = class {
    constructor() {
      this._oldProps = void 0;
    }
    mount(target) {
  ...
  export var HanziWriterRenderer = class {
  ...
  export var RenderTargetBase = class {
    constructor(node) {
      this.node = node;
    }
  ...
  export var RenderTarget = class _RenderTarget extends RenderTargetBase {
  ...
  export var svgRenderer = {
    HanziWriterRenderer,
    createRenderTarget: RenderTarget.init
  };
  export var drawPath = (ctx, points) => {
    ctx.beginPath();
  ...
  export var pathStringToCanvas = (pathString) => {
  ...
  export var StrokeRenderer$1 = class extends StrokeRendererBase {
  ...
  export var CharacterRenderer$1 = class {
    constructor(character) {
  ...
  export function renderUserStroke(ctx, props) {
    if (props.opacity < 0.05) {
      return;
    }
    const {
  ...
  export var HanziWriterRenderer$1 = class {
    constructor(character, positioner) {
      this.destroy = noop;
      this._character = character;
      this._positioner = positioner;
  ...
  export var RenderTarget$1 = class _RenderTarget$1 extends RenderTargetBase {
    constructor(canvas) {
      super(canvas);
    }
    static init(elmOrId, width = "100%", height = "100%") {
  ...
  export var getCharDataUrl = (char) => `https://cdn.jsdelivr.net/npm/hanzi-writer-data@${VERSION}/${char}.json`;
  export var defaultCharDataLoader = (char, onLoad, onError) => {
  ...
  export var LoadingManager = class {
    constructor(options) {
      this._loadCounter = 0;
  ...
  export var HanziWriter = class _HanziWriter {
  ...
  export var hanzi_writer_esm_default = HanziWriter;
